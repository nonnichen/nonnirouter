module("luci.model.cbi.passgfw.api.hysteria", package.seeall)
...
function to_move(file)
    ...
    local flag = sys.call('pgrep -af "passgfw/.*hysteria" >/dev/null')
    if flag == 0 then
        sys.call("/etc/init.d/passgfw stop")
    end
    ...
    sys.call("/bin/rm -rf /tmp/hysteria_download.*")
    if flag == 0 then
        sys.call("/etc/init.d/passgfw restart >/dev/null 2>&1 &")
    end
