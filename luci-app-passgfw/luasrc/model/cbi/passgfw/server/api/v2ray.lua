module("luci.model.cbi.passgfw.server.api.v2ray", package.seeall)
...
function gen_config(user)
    ...
    if user.transit_node and user.transit_node ~= "nil" then
        local transit_node_t = uci:get_all("passgfw", user.transit_node)
        ...
        local outbound = require("luci.model.cbi.passgfw.api.gen_v2ray").gen_outbound(transit_node_t, "transit")
        if outbound then
    ...
    local config = {
        log = {
            -- error = "/tmp/etc/passgfw_server/log/" .. user[".name"] .. ".log",
